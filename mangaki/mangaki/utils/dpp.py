...
def build_matrix(fname=None):
    ...
    return matrix, item_set, user_set, item_dict, user_dict
...
def diameter(r, points):
    ...
    return ((2/(nb_points*(nb_points-1))*((pdist(points)**r).sum()))**(1/r)) 
...
class SimilarityMatrix(object):
    ...
    def __init__(self, matrix, nb_components_svd=10, fname=None, algo='svd', metric='cosine'):
    ...
    def make_svd_matrix(self):
        self.U, self.sigma, self.VT = randomized_svd(self.matrix, self.nb_components_svd)
...
def compare(similarity, algos, nb_points, nb_iterations=100):
...
if __name__ == '__main__':
    matrix, item_set, user_set, item_dict, user_dict = build_matrix(fname='/home/voisin/Bureau/ratings.csv')
    ...
    algos=[uniform, dpp]
    # results = compare3(similarity, algos, 10)
