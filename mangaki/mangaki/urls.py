...
urlpatterns = [
    ...
    url(r'^getuser/(?P<work_id>\w+)\.json$',
        views.get_user_for_recommendations),
    url(r'^getuser\.json$', views.get_users),
    url(r'^recommend/(?P<work_id>\w+)/(?P<target_id>\w+)$', views.recommend_work),
    url(r'^removeReco/(?P<work_id>\d+)/(?P<username>\w+)/(?P<targetname>\w+)$',
        views.remove_reco),
    ...
    url(r'^anime/$', views.WorkList.as_view(),
        {'category': 'anime'}, name='anime-list'),
    ...
    url(r'^manga/$', views.WorkList.as_view(),
        {'category': 'manga'}, name='manga-list'),
